...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user != null) {
            ...
            //Catch the username as null error.
            if (username == null){
                Toast.makeText(MainActivity.this, "Something went wrong with logging in. Please try again.",
        ...
        //----------------------------------------------------------------------------------------------
        // Streak update service is called to update the streak, every day if the user logins. This is
        // handled in another class, StreakUpdateService.
        //----------------------------------------------------------------------------------------------
        ...
        Log.v("MustSee", "Username" + username);
                ...
                query.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            DatabaseReference flashcardRef = categoryRef.child(flashcard.getTitle());  // Use the unique key generated by push()
    ...
    private void retrieveUserCreatedFlashcards(String username) {
        DatabaseReference usersRef = FirebaseDatabase.getInstance().getReference("users");

        usersRef.child(username).child("categories").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot categorySnapshot : dataSnapshot.getChildren()) {
                    ...
                    // Iterate through all flashcards in the category
                    for (DataSnapshot flashcardSnapshot : categorySnapshot.getChildren()) {
                        ...
                        // Add the retrieved flashcard to the flashcardList
                        flashcardList.add(flashcard);
                        Log.d("QuoteApp", "Flashcard retrieved with title: " + flashcard.getTitle());
                    }
                }
    ...
    //----------------------------------------------------------------------------------------------
    //On resume method is called when the user returns to the main page. We want to keep the flashcards
    //the user created for oneself.
    //----------------------------------------------------------------------------------------------
    @Override
    protected void onResume() {
        ...
        flashcardList.clear(); // Clear the existing list to avoid duplicates
        CreateDefaultFc createHandler = new CreateDefaultFc();
        List<Flashcard> defaultFlashcards = createHandler.createFlashcards();
        flashcardList.addAll(defaultFlashcards);

        // Load user-created flashcards from Firebase and add them to flashcardList
        retrieveUserCreatedFlashcards(username);
        displayAllFlashcards();
        // Update the RecyclerView adapter
        fcAdapter.notifyDataSetChanged();
    }
    ...
    private void showStep3Layout() {
        ...
        btnAddQuestion.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // Check if both question and answer are not empty and not exceeding 15 words
                if (!question.isEmpty() && !answer.isEmpty() && isWithinWordLimit(question) && isWithinWordLimit(answer)) {
                    ...
                    Toast.makeText(MainActivity.this, "Both question and answer must be filled and within 15 words", Toast.LENGTH_SHORT).show();
                }
            }
        });
        // Handle the "Next" button click
        btnNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // Check if at least four question and answer sets are added
                if (questionsList.size() < 4) {
                    Toast.makeText(MainActivity.this, "Flashcard must have at least four questions and answers", Toast.LENGTH_SHORT).show();
                    return;
                }
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        for (EditText editText : questionEditTexts) {
                            ...
                            if (!editedQuestion.isEmpty()) {
                                ...
                                Toast.makeText(MainActivity.this, "Questions cannot be empty and must be within 15 words", Toast.LENGTH_SHORT).show();
                                return;
                            }
                        }
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        for (EditText editText : answerEditTexts) {
                            ...
                            if (!editedAnswer.isEmpty()) {
                                ...
                                Toast.makeText(MainActivity.this, "Answers cannot be empty and must be within 15 words", Toast.LENGTH_SHORT).show();
                                return;
                            }
                        }
    ...
    //----------------------------------------------------------------------------------------------
    //Check if the question or answer is within less than 200 characters. This is to prevent the
    //flashcard from being too long, occupying the screen too much.
    //----------------------------------------------------------------------------------------------
    private boolean isWithinWordLimit(String text) {
        return text.length() <= 199;
    }
    ...
    //----------------------------------------------------------------------------------------------
    //Display flashcards by the filtered category, instead of all of them at once.
    //----------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------
    //Handles and displays all flashcards at once.
    //----------------------------------------------------------------------------------------------
    private void displayAllFlashcards() {
        RecyclerView recyclerViewLearnYourself = findViewById(R.id.recyclerView1);
        RecyclerView recyclerViewTestYourself = findViewById(R.id.recyclerView2);

        // Create and set the adapter for Learn Yourself RecyclerView
        FlashcardAdapter fcAdapterLearnYourself = new FlashcardAdapter(flashcardList);
        LinearLayoutManager layoutManagerLearnYourself = new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false);
        recyclerViewLearnYourself.setLayoutManager(layoutManagerLearnYourself);
        int spacingInPixels = 4;
        recyclerViewLearnYourself.addItemDecoration(new SpaceItemDeco(spacingInPixels));
        recyclerViewLearnYourself.setItemAnimator(new DefaultItemAnimator());
        recyclerViewLearnYourself.setAdapter(fcAdapterLearnYourself);

        fcAdapterLearnYourself.setOnItemClickListener(new FlashcardAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(Flashcard flashcard) {
                // Start FlashCardQuestionPage activity with the selected flashcard
                Intent intent = new Intent(MainActivity.this, LearnYourself.class);
        ...
        // Create and set the adapter for Test Yourself RecyclerView
        FlashcardAdapter fcAdapterTestYourself = new FlashcardAdapter(flashcardList);
        LinearLayoutManager layoutManagerTestYourself = new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false);
        recyclerViewTestYourself.setLayoutManager(layoutManagerTestYourself);
        recyclerViewTestYourself.addItemDecoration(new SpaceItemDeco(spacingInPixels));
        recyclerViewTestYourself.setItemAnimator(new DefaultItemAnimator());
        recyclerViewTestYourself.setAdapter(fcAdapterTestYourself);

        fcAdapterTestYourself.setOnItemClickListener(new FlashcardAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(Flashcard flashcard) {
                AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);
                builder.setTitle("Test Yourself");
